...
import {
  ...
  ListItemText, Box, IconButton, Avatar, Divider, Tooltip, Button,
...
const nav = [
  { label: "Dashboard", to: "/", perm: null, icon: <DashboardIcon /> },
  { label: "Admin • Users", to: "/admin/users", perm: PERMS.RBAC_MANAGE, icon: <PeopleIcon /> },
  { label: "Admin • Roles", to: "/admin/roles", perm: PERMS.RBAC_MANAGE, icon: <SecurityIcon /> },
  { label: "Admin • Fields", to: "/admin/fields", perm: PERMS.FIELDS_MANAGE, icon: <TuneIcon /> },
  { label: "Dealers", to: "/dealers", perm: PERMS.DEALERS_READ, icon: <StoreIcon /> },
  { label: "Add Dealers", to: "/dealers/new", perm: PERMS.DEALERS_WRITE, icon: <PersonAddIcon /> },
  { label: "Financial Institutes", to: "/fin", perm: PERMS.FIN_READ, icon: <StoreIcon /> },
  { label: "Add Financial Institute", to: "/fin/new", perm: PERMS.FIN_WRITE, icon: <PersonAddIcon /> },
  { label: "Landlords", to: "/landlords", perm: PERMS.LAND_READ, icon: <StoreIcon /> },
  { label: "Add Landlord", to: "/landlords/new", perm: PERMS.LAND_WRITE, icon: <PersonAddIcon /> },
  { label: "Customers", to: "/customers", perm: PERMS.CUSTOMERS_READ, icon: <GroupIcon /> },
  { label: "Docs", to: "/docs", perm: PERMS.DOCS_READ, icon: <DescriptionIcon /> },
];

// NEW: products & lenders submenu items
const productItems = [
  { key: "ev", label: "EV Product" },
  { key: "mobile-loan", label: "Mobile Loan" },
  { key: "education-loan", label: "Education Loan" },
];
const lenderItems = [
  { key: "ev", label: "EV" },
  { key: "adikosh", label: "Adikosh" },
  { key: "gq-fsf", label: "GQ fsf" },
  { key: "gq-nonfsf", label: "GQ nonFsf" },
  { key: "bl", label: "BL" },
];
...
function permFor(section, mod, action = 'READ') {
  const seg = section === 'product' ? PROD_KEY[mod] : LEND_KEY[mod];
  if (!seg) return null;
  return `${section === 'product' ? 'PROD' : 'LEND'}_${seg}_${action}`; // e.g. PROD_MOBILE_READ
}
...
function hasPerm(user, perm) {
  ...
  if (arr.some(p => p === perm || (p && (p.code === perm || p.name === perm)))) return true;
...
export default function AppLayout({ children }) {
  ...
  const [openLender, setOpenLender] = React.useState(true);

  const can = (perm) => hasPerm(user, perm);
  ...
  const drawerContent = (
    <Box sx={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      <List
        sx={{ px: 1 }}
        subheader={<ListSubheader sx={{ fontWeight: 700 }}>LOS System</ListSubheader>}
      >
        {/* Top-level items */}
        {nav.filter((i) => can(i.perm)).map((i) => {
          const selected =
          ...
          return (
            ...
            >
              <ListItemIcon sx={{ minWidth: 40 }}>{i.icon}</ListItemIcon>
              <ListItemText primary={i.label} primaryTypographyProps={{ fontSize: 14 }} />
            </ListItemButton>
          );
        })}

        {/* ===== Product group (permission-aware) ===== */}
        {(() => {
          const visible = productItems
            .map(it => ({ ...it, perm: permFor('product', it.key, 'READ') }))
          ...
          return (
            <>
              <ListItemButton onClick={() => setOpenProduct(p => !p)} sx={{ mt: 1, borderRadius: 2 }}>
                <ListItemIcon><Inventory2Outlined /></ListItemIcon>
                <ListItemText primary="Product" primaryTypographyProps={{ fontWeight: 600 }} />
                {openProduct ? <ExpandLess /> : <ExpandMore />}
              </ListItemButton>
              <Collapse in={openProduct} timeout="auto" unmountOnExit>
                <List component="div" disablePadding>
                  {visible.map((it) => {
                    const to = `/product/${it.key}/pending`; // land on data
                    const selected = location.pathname.startsWith(`/product/${it.key}`);
                    return (
                      <ListItemButton
                        key={it.key}
        ...
        {/* ===== Lender group (permission-aware) ===== */}
        {(() => {
          const visible = lenderItems
            .map(it => ({ ...it, perm: permFor('lender', it.key, 'READ') }))
            .filter(it => can(it.perm));
          ...
          return (
            <>
              <ListItemButton onClick={() => setOpenLender(p => !p)} sx={{ mt: 0.5, borderRadius: 2 }}>
                <ListItemIcon><AccountBalanceOutlined /></ListItemIcon>
                <ListItemText primary="Lender" primaryTypographyProps={{ fontWeight: 600 }} />
                {openLender ? <ExpandLess /> : <ExpandMore />}
              </ListItemButton>
              <Collapse in={openLender} timeout="auto" unmountOnExit>
                <List component="div" disablePadding>
                  {visible.map((it) => {
                    const to = `/lender/${it.key}/pending`; // land on data
                    const selected = location.pathname.startsWith(`/lender/${it.key}`);
                    return (
                      <ListItemButton
                        key={it.key}
                        sx={{ pl: 6, borderRadius: 2 }}
  ...
  return (
    <Box sx={{ display: "flex", bgcolor: "background.default", minHeight: "100vh" }}>
      ...
      >
        <Toolbar sx={{ px: 2, gap: 1 }}>
          <IconButton onClick={() => setMobileOpen(!mobileOpen)} sx={{ display: { md: "none" } }} edge="start">
            <MenuIcon />
          </IconButton>
          <Typography variant="h6" sx={{ fontWeight: 700, letterSpacing: 0.5 }}>
            LOS
          ...
          <Box sx={{ ml: "auto", display: "flex", alignItems: "center", gap: 1.5 }}>
            ...
            <Typography noWrap sx={{ display: { xs: "none", sm: "block" } }} title={user?.email}>
              {user?.email}
            </Typography>
            <Button size="small" variant="outlined" onClick={logout} sx={{ textTransform: "none", backgroundColor: "red", color: "white" }} startIcon={<LogoutIcon />}>
              Logout
            </Button>
          </Box>
        </Toolbar>
